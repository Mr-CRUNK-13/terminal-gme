...
# --- 3. TERMINAL ---
    ...
    # --- ÉCRAN 1 : GME (Couleurs corrigées) ---
    with tab1:
        ...
        # Le h3 utilise bien la variable 'color'
        st.markdown(f"<div style='display: flex; justify-content: center; align-items: center; gap: 40px; margin-top: 30px;'><div style='text-align: right;'><h1 style='font-size: 100px; color: {color}; text-shadow: 0 0 20px {color}; margin: 0;'>${p_nsy:.2f}</h1><h3 style='color: {color}; margin: 0;'>{pct:+.2f}%</h3></div>{icon}</div>", unsafe_allow_html=True)
    ...
    # --- ÉCRAN 2 : WARRANTS (Fusée réintégrée & Couleurs corrigées) ---
    with tab2:
        pct_w = ((p_wt - prev_wt) / prev_wt) * 100
        col_w = "#00E676" if pct_w >= 0 else "#FF3D00"
        size_w = min(100 + (abs(pct_w) * 10), 200)
        # Réintégration de la logique d'icône (Fusée/Image)
        ...
        # Structure flexbox identique à l'écran 1
        st.markdown(f"<div style='display: flex; justify-content: center; align-items: center; gap: 40px; margin-top: 30px;'><div style='text-align: right;'><h1 style='font-size: 100px; color: {col_w}; text-shadow: 0 0 20px {col_w}; margin: 0;'>${p_wt:.2f}</h1><h3 style='color: {col_w}; margin: 0;'>{pct_w:+.2f}%</h3></div>{icon_w}</div>", unsafe_allow_html=True)
    ...
    # --- ÉCRAN 3 : CAMEMBERT EU (Footer parfaitement centré) ---
    with tab3:
        ...
        texts = {"NSY": build_label("Action GME (NSY)", v_n_e, q_n, p_nsy, "$", p_n, "$", pl_n, (pl_n/(v_n_e-pl_n))*100 if (v_n_e-pl_n)!=0 else 0), "Warrant": build_label("Warrant (NSY)", v_w_e, q_w, p_wt, "$", p_w, "$", pl_w, (pl_w/(v_w_e-pl_w))*100 if (v_w_e-pl_w)!=0 else 0), "XET": build_label("Action GME (XET)", v_x_e, q_x, p_xet, "€", p_x, "€", pl_x, (pl_x/(v_x_e-pl_x))*100 if (v_x_e-pl_x)!=0 else 0), "TDG": build_label("Action GME (TDG)", v_t_e, q_t, p_xet, "€", p_t, "€", pl_t, (pl_t/(v_t_e-pl_t))*100 if (v_t_e-pl_t)!=0 else 0)}
        ...
        for i, k in enumerate(["XET", "NSY", "Warrant", "TDG"]):
            ...
            ax3.annotate(texts[k], xy=(np.cos(np.deg2rad(ang)), np.sin(np.deg2rad(ang))), xytext=pos_dict[k], color=['#FF3D00', '#00E676', '#008000', '#D50000'][i], fontsize=30, fontweight='bold', arrowprops=dict(arrowstyle="-", color=['#FF3D00', '#00E676', '#008000', '#D50000'][i], lw=4), bbox=dict(boxstyle="round,pad=0.5", fc=bg, ec="gray", lw=2), ha="center", va="center")

        # --- FOOTER CENTRÉ ---
        # Coordonnées X ajustées autour de 0.5 pour un centrage parfait
        plt.figtext(0.48, 0.08, "Valeur Totale: ", color="white", fontsize=38, ha="right", weight="bold")
        plt.figtext(0.48, 0.08, f"€{total_e:,.2f} (${total_e*rate:,.2f})", color="#00E676", fontsize=38, ha="left", weight="bold")
        plt.figtext(0.48, 0.04, "Total Actions: ", color="white", fontsize=38, ha="right", weight="bold")
        plt.figtext(0.48, 0.04, f"{q_x+q_t+q_n} | Total Warrants: {q_w}", color="#00E676", fontsize=38, ha="left", weight="bold")
        plt.figtext(0.50, 0.00, "P/L Latent Estimé: ", color="white", fontsize=38, ha="right", weight="bold")
        plt.figtext(0.50, 0.00, f"{pl_tot:+,.2f}€ (${pl_tot*rate:+,.2f})", color="#00E676", fontsize=38, ha="left", weight="bold")
        # Rectangle centré (x=0.25, width=0.5)
        fig3.add_artist(plt.Rectangle((0.25, -0.04), 0.5, 0.17, transform=fig3.transFigure, facecolor=bg, edgecolor="white", lw=3, zorder=-1))
        st.pyplot(fig3)

    # --- ÉCRAN 4 : US ALLOCATION (Texte central ajusté) ---
    with tab4:
        ...
        # TAILLE DE POLICE RÉDUITE À 48 POUR ÉVITER LE DÉBORDEMENT
        ac.text(0, -0.25, f"{t_pl:+,.2f} ({t_pl/(c_s+c_w):+.2%})", fontsize=48, color="#00E676" if t_pl>=0 else "#FF3D00", ha="center", weight="bold")
    ...
    # --- ÉCRAN 5 : DATA TABLE (Parfait, on ne touche plus) ---
    with tab5:
        ...
        rows = [["GME (US)",f"{q_n:,}",f"${pru_n:.2f}",f"${p_nsy:.2f}",f"${c_s:,.2f}",f"${v_s:,.2f}",f_data(v_s, c_s),f"{v_s/t_v:.1%}"],["GME-WT",f"{q_w:,}",f"${pru_w:.3f}",f"${p_wt:.2f}",f"${c_w:,.2f}",f"${v_w:,.2f}",f_data(v_w, c_w),f"{v_w/t_v:.1%}"],["","","","","","","",""],["TOTAL PORTFOLIO","","","",f"${c_s+c_w:,.2f}",f"${t_v:,.2f}",f_data(t_v, c_s+c_w),"100%"]]
        ...
        for (r, c), cell in table.get_celld().items():
            if r == 0: cell.set_facecolor("#001f3f"); cell.get_text().set_color("#00E676"); cell.get_text().set_fontweight('bold')
                ...
                elif c == 6: pl_val = (v_s-c_s) if r==1 else (v_w-c_w); cell.get_text().set_color("#00E676" if pl_val>=0 else "#ef4444"); cell.get_text().set_fontsize(15)
            elif r == 4:
                if c in [0, 4, 5, 6]:
                    ...
                    elif c == 6: cell.get_text().set_color("#00E676" if t_pl>=0 else "#ef4444"); cell.get_text().set_fontsize(15)
